          ...
          if (isset($_POST['recover_pass'])) {
            ...
            if ($db) {
              ...
              $answer=$_POST['q_answer'];
                ...
                $e=mysqli_query($db ,"SELECT teacher_id,teacher_email , security_question,question_answer FROM teachers WHERE teacher_email='$email' AND security_question='$question' AND question_answer='$answer'");
                $status=mysqli_num_rows($e);
                if ($status==1) {
                  $r=mysqli_fetch_array($e);
                  /// href
                  ?><script> window.location.href="p_set1.php?i=<?php echo $r[0]; ?>&q=<?php echo $question; ?>" </script>
                   <?php
                }else {
                  $status_email=mysqli_num_rows(mysqli_query($db ,"SELECT teacher_email FROM teachers WHERE teacher_email='$email'"));
                  if ($status_email==1) {
                    $sql=mysqli_query($db ,"SELECT teacher_email , security_question,question_answer FROM teachers WHERE teacher_email='$email' AND security_question='$question' AND question_answer='$answer'");
                    $status=mysqli_num_rows($sql);
                    if ($status==1) {  /// href
                      $r=mysqli_fetch_array($status);
                        ?><script> window.location.href="p_set1.php?i=<?php echo $r[0]; ?>&q=<?php echo $question; ?>" </script> <?php
                    }else {
...
<h4 style="color: white; margin-top: 10px;padding-top: 5px;margin-left: 5px;letter-spacing: 1px; "> Teacher Registeration</h4>    
    </div>
